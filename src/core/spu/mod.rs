...
impl SoundRam {
    ...
    fn write_irq_address(&mut self, value: u32) {
        ...
        debug!("SPU IRQ address: {:05X}", self.irq_address);
    }
...
impl DataPort {
    ...
    fn write_transfer_address(&mut self, value: u32) {
        ...
        debug!("Sound RAM data transfer address: {:05X}", self.start_address);
    }
}
...
impl ControlRegisters {
    ...
    fn write_spucnt(
        ...
        debug!("SPUCNT write");
        debug!("  Soft reset: {}", self.soft_reset);
        debug!("  Amplifier enabled: {}", self.amplifier_enabled);
        debug!("  Noise shift: {}", noise.shift);
        debug!("  Noise step: {}", noise.step + 4);
        debug!("  Reverb writes enabled: {}", reverb.writes_enabled);
        debug!("  IRQ enabled: {}", sound_ram.irq_enabled);
        debug!("  Data port mode: {:?}", data_port.mode);
        debug!("  External audio reverb enabled: {}", self.external_audio_reverb_enabled);
        debug!("  CD audio reverb enabled: {}", reverb.cd_enabled);
        debug!("  External audio enabled: {}", self.external_audio_enabled);
        debug!("  CD audio enabled: {}", self.cd_audio_enabled);
    }
}
...
impl DmaDevice for Spu {
    ...
    fn dma_read(&mut self) -> u32 {
        let low_halfword = self.read_data_port() as u32;
...
impl Spu {
    ...
    pub fn clock(
        ...
        if !self.last_irq_bit && sound_ram_irq {
            irq_handler.set_irq(InterruptType::SPU);
        }
    ...
    pub fn read_register<const SIZE: usize>(&mut self, address: u32) -> u32 {
        const { assert!(SIZE == 8 || SIZE == 16 || SIZE == 32)}
        
        debug!("SPU register read: {address:08X}");

        if SIZE == 32 {
            let low_halfword = self.read_register::<16>(address);
            let high_halfword = self.read_register::<16>(address | 2);
        ...
        match SIZE {
            ...
            16 => value,
            _ => unreachable!("size Word should have early returned"),
        }
    }
    ...
    pub fn write_register<const SIZE: usize>(&mut self, address: u32, value: u32) {
        const { assert!(SIZE == 8 || SIZE == 16 || SIZE == 32)}

        debug!("SPU register write: {address:08X} {value:08X} {SIZE}");
        ...
        match SIZE {
            ...
            32 => {
                ...
                self.write_register::<16>(address, value & 0xFFFF);
                self.write_register::<16>(address | 2, value >> 16);
                return;
            }
            _ => unreachable!()
        }
        ...
        match address & 0xFFFF {
            ...
            0x1DAC => {
                ...
                if value & 0xFFFF != 0x0004 {
                    error!("Unexpected sound RAM data transfer control write: {value:04X}");
                }
            }
            0x1DAE => {
    ...
    fn read_voice_register(&self, address: u32) -> u32 {
        ...
        if voice >= NUM_VOICES {
            error!("Invalid voice register read: {address:08X}");
            return 0;
        }
        ...
        match address & 0xF {
            ...
            _ => {
                warn!("SPU voice {voice} register read: {address:08X}");
                0
            }
        }
    }
    ...
    fn write_voice_register(&mut self, address: u32, value: u32) {
        let voice = get_voice_number(address);
        if voice >= NUM_VOICES {
            error!("Invalid voice register write: {address:08X} {value:04X}");
            return;
        }
        ...
        match address & 0xF {
            0x0 => {
                ...
                debug!("Voice {voice} volume L: {:?}", self.voices[voice].volume_l);
            }
            0x2 => {
                // $1F801C02 + N*$10: Voice volume R
                self.voices[voice].volume_r.write(value);
                debug!("Voice {voice} volume R: {:?}", self.voices[voice].volume_r);
            }
            0x4 => {
                // $1F801C04 + N*$10: Voice sample rate
                self.voices[voice].sample_rate = value as u16;
                debug!("Voice {voice} sample rate: {:04X}", self.voices[voice].sample_rate);
            }
            0x6 => {
                // $1F801C06 + N*$10: ADPCM start address
                self.voices[voice].write_start_address(value);
                debug!("Voice {voice} start address: {:05X}", (value & 0xFFFF) << 3);
            }
            0x8 => {
                // $1F801C08 + N*$10: ADSR settings, low halfword
                self.voices[voice].adsr.settings.write_low(value);
                debug!("Voice {voice} ADSR settings (low): {:?}", self.voices[voice].adsr);
            }
            0xA => {
                // $1F801C0A + N*$10: ADSR settings, high halfword
                self.voices[voice].adsr.settings.write_high(value);
                debug!("Voice {voice} ADSR settings (high): {:?}", self.voices[voice].adsr);
            }
            0xC => {
                // $1F801C0C + N*$10: Current ADSR level
            ...
            0xE => {
                ...
                debug!("Voice {voice} repeat address: {:05X}", (value & 0xFFFF) << 3);
            }
    ...
    pub fn write_data_port(&mut self, value: u16) {
        ...
        debug!("Wrote to {:05X} in audio RAM", self.data_port.current_address);
    ...
    fn write_key_on(&mut self, voices: Range<usize>, value: u32) {
        debug!("Key on write for voices {voices:?}: {value:04X}");
    ...
    fn write_key_off(&mut self, voices: Range<usize>, value: u32) {
        debug!("Key off write for voices {voices:?}: {value:04X}");
    ...
    fn write_pitch_modulation_low(&mut self, value: u32) {
        debug!("Pitch modulation low write: {value:04X}");
    ...
    fn write_pitch_modulation_high(&mut self, value: u32) {
        debug!("Pitch modulation high write: {value:04X}");
    ...
    fn write_noise_low(&mut self, value: u32) {
        debug!("Noise low write: {value:04X}");
    ...
    fn write_noise_high(&mut self, value: u32) {
        debug!("Noise high write: {value:04X}");
