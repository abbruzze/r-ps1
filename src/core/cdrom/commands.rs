...
use crate::core::cdrom::disc::{DiscTime, TrackType, BCD};
use crate::core::clock::ClockConfig;
...
const STAT_NO_ERR : Option<(bool,bool,bool)> = Some((false,false,false));
const STAT_NO_DATA : &[u8] = &[];

const FIRST_RESPONSE_IRQ_DELAY_44100 : usize = 1;
const STD_SECOND_RESPONSE_IRQ_DELAY_44100 : usize = delay_cycles_44100(0x4A73);
const PAUSE_RESPONSE_IRQ_DELAY_44100 : usize = delay_cycles_44100(0x10BD93);
...
impl CDRom {
    pub(super) fn check_drive_state(&mut self,irq_handler: &mut IrqHandler) {
        ...
        let new_state = match state {
            ...
            DriveState::Playing => DriveState::Playing,
    ...
    fn apply_irq_and_result(&mut self, irq:u8, response:Vec<u8>, irq_handler: &mut IrqHandler) {
        self.set_irq(irq);
    ...
    fn get_approx_seek_cycles_44100(&self,from:&DiscTime,target:&DiscTime) -> usize {
        ...
        self.get_cycles_per_ms_44100(seek_time_ms as f32)
    }

    #[inline(always)]
    fn get_cycles_per_ms_44100(&self,cycles:f32) -> usize {
        ((ClockConfig::NTSC_CPU_CLOCK as f32 / 44100.0 / 1000.0 * cycles) as usize).max(1)
    }
    ...
    fn command_read(&mut self) -> CommandState {
        if let Some(disc) = self.disc.as_mut() {
            ...
            self.drive_state = DriveState::Reading { next_sector_cycles };
            self.activate_motor(true);
    ...
    fn command_pause(&mut self,second_response:bool) -> CommandState {
        if second_response {
            ...
            self.make_response(
                ...
                CommandState::Delay {
                    cmd: Command::Pause,
                    delay_cycles: PAUSE_RESPONSE_IRQ_DELAY_44100,
                    next_state: Box::new(
