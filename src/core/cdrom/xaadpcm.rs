...
impl XaAdpcmState {
    ...
    pub fn decode_sector(&mut self, sector: &[u8]) {
        ...
        self.channel_mode =
            if (coding_info & 0x01) != 0 { ChannelMode::Stereo } else { ChannelMode::Mono };

        self.sample_rate = if (coding_info & 0x04) != 0 { SampleRate::Half } else { SampleRate::Normal };
            ...
            if counter == 6 {
                ...
                for table in 0..7 {
                    ...
                    for i in 1..30 {
                        ...
                        let table_value: i32 = INTERPOLATION[7 * (i - 1) + table].into();
