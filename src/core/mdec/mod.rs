...
impl DecodeConfig {
    fn from_command(command: u32) -> Self {
        Self {
            ...
            signed: (command & (1 << 26)) != 0,
            bit_15: (command & (1 << 25)) != 0,
...
impl MDecIn {
    pub fn new(mdec: &Rc<RefCell<MDec>>) -> Self {
        Self { mdec: mdec.clone() }
    }
}

impl DmaDevice for MDecIn {
    ...
    fn dma_write(&mut self, word: u32, _clock: &mut Clock, _irq_handler: &mut IrqHandler) {
        self.mdec.borrow_mut().write_command(word);
    }
...
impl DmaDevice for MDecOut {
    ...
    fn dma_read(&mut self) -> u32 {
        self.mdec.borrow_mut().read_data()
    }
}
...
impl MDec {
    ...
    pub fn write_command(&mut self, value: u32) {
        debug!("MDEC command write: {value:08X}");
        ...
        self.command_state = match self.command_state {
            CommandState::Idle => match value >> 29 {
                ...
                1 => {
                    ...
                    debug!("MDEC decode command: {value:08X}");
                    debug!("  Decode config: {:?}", self.decode_config);
    ...
    fn decode_colored_macroblocks(&mut self) {
        ...
        debug!("Decoded {count} 16x16 colored macroblocks");
    }
    ...
    fn decode_monochrome_macroblocks(&mut self) {
        ...
        debug!("Decoded {count} 8x8 monochrome macroblocks");
    }

    fn populate_luminance_table(&mut self) {
        debug!("Populating luminance quant table");
    ...
    fn populate_color_table(&mut self) {
        debug!("Populating color quant table");
    ...
    pub fn read_status(&self) -> u32 {
        ...
        debug!("MDEC status read: {value:08X}");
    ...
    pub fn write_control(&mut self, value: u32) {
        ...
        self.enable_data_in = (value & (1 << 30)) != 0;
        self.enable_data_out = (value & (1 << 29)) != 0;

        debug!("MDEC control write: {value:08X}");
        debug!("  MDEC reset: {}", (value & (1 << 31)) != 0);
        debug!("  Data in request enabled: {}", self.enable_data_in);
        debug!("  Data out request enabled: {}", self.enable_data_out);
    }
}
...
fn decode_block(
    ...
    loop {
        ...
        if q_scale > 0 {
            block[ZAG_ZIG[k as usize] as usize] = value;
        } else {
